...
export default function Form() {
    ...
    return (
        <div>
            {info ? <div className="flex items-center p-5 gap-5 text-inkBlue bg-white text-center text-3xl">
                <p className="">{info}</p> <i className="fa-solid fa-circle-check mt-[.233rem]"></i>
            </div> :
                <div className="hidden"></div>}
            <form className={`flex flex-col gap-5 ${info || errorCheck ? 'hidden' : ''}`} onSubmit={handleSubmit}>
                <input
            ...
            {errorCheck ? <div className="mx-auto translate-x-12 flex items-center gap-5 text-inkBlue bg-white text-center fixed top-[10.5rem] text-lg px-2 py-2 rounded left-[.2rem]">
